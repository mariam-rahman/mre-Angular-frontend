...
export class RestoreComponent implements OnInit {
  ...
  @Output() restoreEventEmiter = new EventEmitter<Object>();
@Input() productId;
@Input() qty;
  ...
  onFormSubmit() {
    if (this.newForm.valid) {
      ...
      if (move_qty <= this.qty) {
        this.subStockSerivce. restoreData({ move_qty,discount,sell_price, product_id: this.productId }).subscribe((response) => {
          console.log('MoveTo Sub response', response);
          if (response == true) {
            this.toaster.success("SUCCESS", "RECORD_CREATED_SUCCESSFULLY");

            this.restoreEventEmiter.emit(response);
            this.closeModal();
          } else {
