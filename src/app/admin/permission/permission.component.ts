...
export class PermissionComponent implements OnInit, OnDestroy {
	...
	addNew() {
		...
		// {
			const modalRef = this.modalService.open(CreatePermissionComponent);
			modalRef.componentInstance.permissionCreateEventEmitter.subscribe(() => {	
				this.reloadData();
			});
	...
	deleteRecord(id) {
		...
		const modalRef = this.modalService.open(DeletePermissionComponent);
	     modalRef.componentInstance.id = id;
		 modalRef.componentInstance.permissionDeleteEventEmitter.subscribe((res)=>{
			 this.reloadData();
		 });
	...
	editRecord(id){
		this.permissionService.getpermissionForEdit(id).subscribe(res => {
			const modalRef = this.modalService.open(EditPermissionComponent);
			modalRef.componentInstance.data = res;
		
			modalRef.componentInstance.permissionEditEventEmitter.subscribe((res) => {
				this.reloadData();
			});
		},(error)=>{ 
